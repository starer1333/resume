...
test("hero has six personal fragments and replays after every upward re-arm", async ({ page }) => {
  ...
  await expect(hero).toHaveAttribute("data-hero-state", "collapsed");
...
test("Observe is immediately final and interactive with no counter gate", async ({ page }) => {
  ...
  await expect(page.locator(".observe-carousel > div")).toHaveCount(4);
